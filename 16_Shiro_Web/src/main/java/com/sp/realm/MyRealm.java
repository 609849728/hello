...
public class MyRealm extends AuthorizingRealm {
    ...
    /*
    授权，登录验证成功后是不会进入此方法的
    只有需要判断是否具有权限时，才会进入，且必须是在已经成功登录的情况下
     */
    @Override
    protected AuthorizationInfo doGetAuthorizationInfo(PrincipalCollection principalCollection) {
        ...
        //通过Dao获取该用户的角色和权限（通过用户名获取并且返回Set集合）
        ...
        //把通过dao获取到的角色和权限放进去，参数类型都是Set<String>，所以才是获取的角色和权限都是返回Set<String>
    ...
    protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken authenticationToken) throws AuthenticationException {
        ...
        if(passwordDB==null || !password.equals(passwordDB)) {
            throw new AuthenticationException();  //密码错误，抛出异常
        }


        //认证成功！认证信息里存放账号密码, getName() 是当前Realm的继承方法,通常返回当前类名 :databaseRealm
