...
function getParentZTree(){
    ...
    htmlStr += '<div style=" line-height:30px; text-indent:10px; margin-bottom:20px; background-color:#eee; position:relative;">目录</div>';
    htmlStr += '<div style="display:block; padding-bottom:5px;" align="center" >';
    htmlStr += '<div><table><tr><td><input type="hidden" id="id" width="50px"/><input type="text" id="name" width="50px"><button onclick="SetQueryMenuTreeParam()">确定</button></td></tr></table></div>';
...
function init() {
    ...
    var setting = {
        ...
        async: {
            ...
            url: "MenuController/getMenuTree",  //访问的url
            autoParam: ["id"],  //参数
...
function SetQueryMenuTreeParam() {
    $("#menuParentName").val($("#name").val());
    $("#menuParentId").val($("#id").val());
    $("#menuType").val("0");  //给类型赋值，0代表菜单，1代表目录
